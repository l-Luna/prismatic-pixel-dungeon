...
public class TerrainFeaturesTilemap extends DungeonTilemap {
	...
	protected int getTileVisual(int pos, int tile, boolean flat){
		...
		if(pressurePads.get(pos) != null){
			PressurePad pad = pressurePads.get(pos);
			return pad.colour + ((pad.pressed ? 9 : 8)*16);
		}
		...
		if (tile == Terrain.HIGH_GRASS){
			...
			return 14 + 16*stage + (DungeonTileSheet.tileVariance[pos] >= 50 ? 1 : 0);
		} else if (tile == Terrain.EMBERS) {
			return 10 * (16*5) + (DungeonTileSheet.tileVariance[pos] >= 50 ? 1 : 0);
		}
